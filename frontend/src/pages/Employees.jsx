...
export default function Employees() {
  ...
  // NEW: track which attendance entry is expanded in mobile view
  const [expandedAttendanceId, setExpandedAttendanceId] = useState(null);

  const [view, setView] = useState(window.innerWidth < 640 ? "mobile" : "desktop");
  ...
  useEffect(() => {
    const handleResize = () => {
      setView(window.innerWidth < 640 ? "mobile" : "desktop");
    };
    handleResize();
  ...
  const openLedger = async (emp, options = {}) => {
    if (options.toggle && selectedEmployee?._id === emp._id) {
  ...
  const deleteMonth = async () => {
    ...
    const confirmDelete = window.confirm("Delete this month attendance?");
  ...
  return (
    <div className="w-full max-w-[100vw] overflow-x-hidden p-4 sm:p-6 text-[color:var(--text-primary)]">
            ...
            {employees
              ...
              .map((emp) => (
                <tr
                  ...
                  onClick={() => openLedger(emp, { toggle: true })}
                >
                  <td>{emp.name}</td>
                  <td>{emp.role}</td>
                  <td>{emp.shift}</td>
      ...
      {/* Mobile employee cards (unchanged) */}
            ...
            return (
              ...
              >
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-white font-semibold text-lg">{emp.name}</p>
                    <p className="text-gray-400 text-sm">
                      {emp.role} • Shift {emp.shift}
                    </p>
                  </div>
                  <p className="text-green-400 font-semibold">₹{emp.salary}</p>
                </div>
                <p className="text-gray-400 text-sm mt-2">📞 {emp.phone}</p>
                <div
      ...
      {/* Desktop attendance section (unchanged) */}
      {view === "desktop" && selectedEmployee && (
        <div className="mt-6 rounded-[28px] border border-[var(--border-strong)] bg-[var(--bg-panel)] p-5 shadow-[0_16px_32px_rgba(16,24,20,0.05)]">
          <div className="mb-4 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <h2 className="text-lg">Attendance : {selectedEmployee.name}</h2>
            <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-soft)] px-4 py-3 text-sm font-medium">
              Total Shortage :{" "}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              />
              <button className="btn btn-green" onClick={() => setAttendanceModal(true)}>
                + Add
              </button>
              <button className="btn btn-purple" onClick={() => setReportOpen(true)}>
                PDF
              </button>
              <button className="btn btn-red" onClick={deleteMonth}>
                Delete Month
              </button>
            </div>
          </div>
          ...
          <div className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="card">Earned : ₹{Math.round(summary.earned)}</div>
            <div className="card">Shortage : ₹{summary.shortage}</div>
            <div className="card">Advance : ₹{summary.advance}</div>
            <div className="card text-lg md:col-span-2 xl:col-span-3">
              Final Balance : ₹{Math.round(summary.final)}
            </div>
          </div>

          <button
            className="mb-3 rounded-xl bg-green-500 px-4 py-2 text-white shadow-sm"
            ...
            <tbody>
              {filteredAttendance.map((a) => (
                <tr key={a._id}>
                  ...
                  <td>{a.status}</td>
                  <td className={Number(a.shortage) >= 0 ? "text-green-400" : "text-red-400"}>
...
{/* ========== MOBILE ATTENDANCE SECTION (UPDATED) ========== */}
{view === "mobile" && selectedEmployee && (
  <div className="mt-6 space-y-4">
    <div className="bg-[#0B0F17] p-4 rounded-xl border border-[#1F2937]">
      ...
      <p className="text-gray-400 text-sm">Attendance Summary</p>
    </div>
    ...
    <div className="space-y-3">
      {filteredAttendance.map((a) => (
        <div
          key={a._id}
          className="bg-[#0B0F17] border border-[#1F2937] rounded-xl overflow-hidden transition-all duration-200 ease-out"
        >
          {/* Clickable header – no black background on tap */}
          <div
            className="p-3 cursor-pointer"
            onClick={() =>
              setExpandedAttendanceId(expandedAttendanceId === a._id ? null : a._id)
            }
          >
            {/* Flex layout: date and short on right side */}
            <div className="flex justify-between items-start">
              <p className="text-gray-400 text-sm">📅 {a.date?.slice(0, 10)}</p>
              {Number(a.shortage) < 0 ? (
                <p className="text-red-400">Short: {a.shortage}</p>
                ) : (
                <p className="text-green-400">+{a.shortage}</p>
                )}
            </div>
            <p className="text-white text-sm mt-1">Status: {a.status}</p>
            <p className="text-sm text-gray-400 mt-1">
              Cash: {a.advanceCash} | Petrol: {a.advancePetrol}
            </p>
          </div>

          {/* Expandable buttons with smooth height transition */}
          <div
            className={`overflow-hidden transition-all duration-200 ease-out ${
              expandedAttendanceId === a._id ? "max-h-20" : "max-h-0"
            }`}
          >
            <div className="flex gap-3 p-3 pt-0 mt-0 border-t border-[#1F2937]">
              <button
                onClick={() => {
                  setEditAttendance(a);
                  setAttendanceModal(true);
                }}
                className="flex-1 mt-2 rounded-full bg-blue-500/20 border border-blue-500/40 text-blue-300 py-2 text-sm font-medium transition hover:bg-blue-500/30"
              >
                Edit
              </button>
              <button
                onClick={() => removeAttendance(a._id)}
                className="flex-1 mt-2 rounded-full bg-red-500/20 border border-red-500/40 text-red-300 py-2 text-sm font-medium transition hover:bg-red-500/30"
              >
                Delete
              </button>
            </div>
